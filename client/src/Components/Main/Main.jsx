...
class Main extends Component {
  ...
  handleUpClick = (index) => {
    if (index !== 0) {
      this.setState((prevState) => {
        let list = [
          ...prevState.currentGame.users.filter(
  ...
  updateAppStateGame = async (newGame) => {
    if (newGame.type === "new_user") {
      ...
      let user = this.state.currentGame.users.findIndex(
        (user) => user.id === newGame.user.id
      );
      let userUpdate = [...this.state.currentGame.users];
      userUpdate[user] = {
        ...userUpdate[user],
        initiative: newGame.user.initiative,
        username: newGame.user.username,
      };
      this.setState({
  ...
  render() {
    return (
      <div className="App">
        ...
        <Route
          exact
          path="/combat/:code/player/:username"
          render={(props) => {
            return this.state.currentGame ? (
              <PlayerCombat
                {...props}
